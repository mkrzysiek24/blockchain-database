...
def generate_public_key_pem(private_key_pem):
...
class User(BaseModel):
    ...
    def generate_keys(cls, values):
        if not values.get("private_key"):
            values["private_key"] = generate_private_key_pem()
        if not values.get("public_key"):
            values["public_key"] = generate_public_key_pem(values.get("private_key"))
        return values
